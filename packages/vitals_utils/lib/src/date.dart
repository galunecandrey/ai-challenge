...
const int kMillisecondsInWeek = 7 * 24 * 60 * 60 * 1000;
...
extension DateTimeExt on DateTime {
  ...
  /// Create a [DateExt] object from a Unix timestamp
  static DateTime unix(int seconds) => fromSecondsSinceEpoch(seconds);

  /// Tomorrow at same hour / minute / second than now
  static DateTime getTomorrow(DateTime current) => current.nextDay;

  /// Yesterday at same hour / minute / second than now
  static DateTime getYesterday(DateTime current) => current.previousDay;

  /// Get [DateExt] object as UTC of current object.
  DateTime get toUTC => toUtc();

  /// Get [DateExt] object in LocalTime of current object.
  ...
  /// Get index of the closest day to current one, returns -1 if empty [Iterable] is passed as argument
  int closestIndexTo(Iterable<DateTime> datesArray) {
    final differences = datesArray.map((date) => date.difference(this).abs());
  ...
  static DateTime getEndOfToday(DateTime current) => current.endOfDay;

  /// Return the end of tomorrow. The result will be in the local timezone.
  static DateTime getEndOfTomorrow(DateTime current) => current.nextDay.endOfDay;

  /// Return the end of yesterday. The result will be in the local timezone.
  static DateTime getEndOfYesterday(DateTime current) => current.previousDay.endOfDay;
  ...
  int get noISOWeekDay => (weekday % DateTime.daysPerWeek) + 1;
  ...
  int get getDayISO => weekday;

  /// Get the day of the week of the given date.
  int get getDay => noISOWeekDay;
  ...
  int get getWeekdayISO => weekday;

  int get getWeekday => noISOWeekDay;

  /// Get the week index
  int get getWeekOfYear => ((startOfWeek.difference(startOfWeekYear).inMilliseconds / kMillisecondsInWeek) + 1).round();

  /// Get the week index
  int get getWeekOfMonth =>
      ((startOfWeek.difference(startOfWeekOfMonth).inMilliseconds / kMillisecondsInWeek) + 1).round();

  /// Get the ISO week index
  int get getISOWeekOfYear =>
      ((startOfISOWeek.difference(startOfISOWeekYear).inMilliseconds / kMillisecondsInWeek) + 1).round();

  /// Get the ISO week index
  int get getISOWeekOfMonth =>
      ((startOfISOWeek.difference(startOfISOWeekOfMonth).inMilliseconds / kMillisecondsInWeek) + 1).round();
  ...
  static bool isDate<T>(T argument) => argument is DateTime;
  ...
  /// Return true if this date [isAfter] [DateExt.now]
  bool isFuture(DateTime current) => isAfter(current);
  ...
  // ignore: comment_references
  /// Return true if this date [isBefore] [DateExt.now]
  bool isPast(DateTime current) => isBefore(current);
  ...
  bool isThisHour(DateTime current) => startOfHour == current.startOfHour;

  /// Check if this date is in the same minute than [DateTime.now]
  bool isThisMinute(DateTime current) => startOfMinute == current.startOfMinute;

  /// Check if this date is in the same month than [DateTime.now]
  bool isThisMonth(DateTime current) => isSameMonth(current);

  /// Check if this date is in the same second than [DateTime.now]
  bool isThisSecond(DateTime current) => isSameSecond(current);

  /// Check if this date is in the same year than [DateTime.now]
  bool isThisYear(DateTime current) => isSameYear(current);

  // ignore: comment_references
  /// Check if this date is in the same day than [DateTime.today]
  bool isToday(DateTime current) => isSameDay(current);

  // ignore: comment_references
  /// Check if this date is in the same day than [DateTime.tomorrow]
  bool isTomorrow(DateTime current) => isSameDay(getTomorrow(current));

  // ignore: comment_references
  /// Check if this date is in the same day than [DateTime.yesterday]
  bool isYesterday(DateTime current) => isSameDay(getYesterday(current));
  ...
  static DateTime getStartOfToday(DateTime current) => current.startOfDay;

  /// Get a [DateTime] representing start of week of this [DateTime] in local time.
  DateTime get startOfWeek => subtract(Duration(days: weekday % DateTime.daysPerWeek)).startOfDay;
  ...
  /// Get the start of a local week-numbering month
  DateTime get startOfWeekOfMonth => startOfMonth.startOfWeek;

  /// Get the start of a local week-numbering month
  DateTime get startOfISOWeekOfMonth => startOfMonth.startOfISOWeek;
  ...
  int get getISOWeeksInYear {
    ...
    final diff = thisYear.difference(nextYear);
    // Round the number of weeks to the nearest integer
    // because the number of milliseconds in a week is not constant
    // (e.g. it's different in the week of the daylight saving time clock shift)
    return (diff.inMilliseconds / kMillisecondsInWeek).round();
  }

  /// Get the number of weeks in an ISO week-numbering year
  int get getISOWeeksInMonth {
    final thisMonth = startOfISOWeekOfMonth;
    final nextMonth = thisMonth.endOfMonth.startOfISOWeekOfMonth;
    final diff = thisMonth.difference(nextMonth);
    // Round the number of weeks to the nearest integer
    // because the number of milliseconds in a week is not constant
    // (e.g. it's different in the week of the daylight saving time clock shift)
    return ((diff.inMilliseconds / kMillisecondsInWeek) + 1).round();
  }

  int get getWeeksInMonth => endOfMonth.startOfWeek.difference(startOfWeekOfMonth).inDays ~/ DateTime.daysPerWeek + 1;

  /// Subtracts a [Duration] from this [DateTime]
  DateTime sub(Duration duration) => subtract(duration);
  ...
  DateTime subDays(int amount) => subtract(Duration(days: amount));
...
extension DurationOperators on Duration {
  ...
  Duration operator *(num other) => Duration(microseconds: (inMicroseconds * other).round());

  Duration operator /(num other) => Duration(microseconds: (inMicroseconds / other).round());
}

extension IntDateTimeExt on int {
  DateTime get dateTime => DateTime.fromMillisecondsSinceEpoch(this);

  DateTime get dateTimeMicro => DateTime.fromMicrosecondsSinceEpoch(this);
}
